...
export default class Staff extends Component {
  ...
  async getVendorID(index) {
    ...
    console.log("purposeValue 1:---------", this.state.purposeValue, this.state.staffId, this.state.vendorName)




    // RNFetchBlob.fs.readFile('/storage/emulated/0/Download/SupportStaff.json', 'utf8')
    //   .then((resp) => {
    //     // console.log("local file read :- ", resp);
    //     const d = JSON.parse(resp);
    //     // console.log("SupportStaff local file read :-  :-", d.data[index - 1].staff)
    //     // console.log("SupportStaff local file read :-  :-", d.data[index - 1].staff.length)
    //     this.setState({
    //       loader: false,
    ...
    //     if (d.data[index - 1].staff.length > 0) {
    //       this.setState({
    //         staffData: d.data[index - 1].staff,
    //         loader: false,
    //         showList: true,
    //         loaderList: false,
    //       })
  ...
  render() {
    return (
      <View animation="fadeInRight" style={styles.container} duration={400}>

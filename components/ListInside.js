...
export default class ListInside extends Component {
  ...
  deletedSelectedItems(i, item) {
    ...
    if (this.state.deletionArray.length === 0) {
      ...
      Alert.alert('', 'Please select something.', [{text: 'Okay'}], {
        cancelable: true,
      });
    } else {
      console.log('22');
  ...
  deleteAllData() {
    ...
    if (this.state.data.length === 0) {
      Alert.alert('', 'No Data Found.', [{text: 'Okay'}], {cancelable: true});
    } else {
      console.log('empty');
      this.selectAll();
    }
  ...
  async deletePer() {
    ...
    fetch(
      `https://ashoka.vizsense.in/api/exit?slipNo=${this.state.deletionArray}`,
      {
        method: 'POST',
        headers: {
          Accept: 'application/json',
        ...
        <View style={styles.cr}>
            ...
            <View
              ...
              <TouchableOpacity
                ...
                <Text style={[styles.title, {fontSize: 18}]}>Exit</Text>
              </TouchableOpacity>

              <TouchableOpacity
                onPress={() => this.deleteAllData()}
                style={[styles.btnd]}>
                <Text style={[styles.title]}>Exit all</Text>
              </TouchableOpacity>
            </View>
                ...
                return (
                  <React.Fragment key={i}>
                    <View
                      ...
                      <View style={{}}>
                        <Text style={[styles.title]}>{item.vSlipNo}</Text>
                        <Text style={[styles.title]}>
                          {item.vFirstName} {item.vLastName}
                        </Text>
                        <Text style={[styles.title]}>
                          {item.dtEntryDate.replace('T', ' ')}
                        </Text>
                      </View>
